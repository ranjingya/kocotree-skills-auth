...
首次运行（无 token）分两步完成：第 1 次调用打印授权链接后脚本退出，用户在浏览器完成授权后，第 2 次调用自动轮询获取 token（每 3 秒，最长 60 秒），保存到 `~/.kocotree-skills/auth.json`。后续请求自动带 token，过期自动刷新。
